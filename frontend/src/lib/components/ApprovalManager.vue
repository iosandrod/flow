<template>
  <div class="approval-manager">
    ...
    <n-modal v-model:show="showStartModal" preset="card" title="发起审批" :style="{ width: '550px' }" :mask-closable="false">
      <n-form ref="formRef" :model="formData" :rules="rules" label-placement="left" label-width="90px">
        ...
        <n-form-item label="关联表名">
          <n-input v-model:value="formData.relatedTable" placeholder="请输入关联表名（选填）" />
        </n-form-item>
        
        <n-form-item label="关联单据号">
          <n-input v-model:value="formData.relatedBillNo" placeholder="请输入关联单据号（选填）" />
        </n-form-item>
...
import { NCard, NGrid, NGi, NSpace, NButton, NIcon, NSelect, NEmpty, NTag, NModal, NForm, NFormItem, NInput, NInputNumber, NDivider, NTabs, NTabPane, NDescriptions, NDescriptionsItem, useMessage } from 'naive-ui'
import { RefreshCw, Plus } from 'lucide-vue-next'
import type { User, WorkflowResponse, Approval, ApprovalHistory } from '../types'
...
async function loadApprovals() {
  ...
  try {
    ...
    if (res.success) {
      approvals.value = (res.data || []).filter((a: Approval) => {
...
async function loadFlowOptions() {
  try {
    ...
    if (res.success && res.data) {
      ...
      })).filter((b: any) => b.value) // Filter out any invalid entries
    }
    formData.value.businessKey = `biz_${Date.now()}`
  } catch (e: any) {
    // Suppress BPMN icon errors
    if (e.message && e.message.includes('image')) {
      console.warn('BPMN icon warning:', e.message)
    } else {
      emit('error', { code: 'LOAD_FLOW_ERROR', message: e.message })
    }
  }
}

function viewDetail(item: Approval) {
  currentApproval.value = item
  showDetailModal.value = true
  loadHistory(item)
}

async function loadHistory(item: Approval) {
  try {
    const res = await props.apiRequest('getProcessHistory', { processId: item.ID_ })
    if (res.success) {
...
async function handleSubmit() {
  ...
  try {
    const data = {
      ...
      starterId: props.currentUser.id,
      businessKey: formData.value.businessKey,
      processName: formData.value.processName || '审批流程',
      relatedTable: formData.value.relatedTable || undefined,
      relatedBillNo: formData.value.relatedBillNo || undefined,
      variables: {
        amount: formData.value.amount,
        reason: formData.value.reason,
        applicant: props.currentUser.username,
        _starterName: props.currentUser.name
      }
    }
    
    const res = await props.apiRequest('start', data)
    ...
    if (res.success) {
      ...
      emit('error', { code: 'SUBMIT_ERROR', message: res.message || '提交失败' })
    }
  } catch (e: any) {
    message.error(e.message || '提交失败')
    emit('error', { code: 'SUBMIT_ERROR', message: e.message })
  } finally {
...
function getStatusText(status?: string): string {
  const map: Record<string, string> = {
    F: '已完成', C: '已关闭', N: '已拒绝', I: '新单', A: '审核中', D: '部分审核'
  }
